...
if(NOT ${IOS_ARCHS_NBR})
  message(FATAL_ERROR "No xros simulator or device architecture to compile for. Populate IOS_DEVICE_ARCHITECTURES and/or IOS_SIMULATOR_ARCHITECTURES.")
endif()

# iOS Deployment Target
execute_process(COMMAND /usr/bin/xcrun -sdk xros --show-sdk-version
                OUTPUT_VARIABLE IOS_DEPLOYMENT_TARGET_TMP
                OUTPUT_STRIP_TRAILING_WHITESPACE)
set(IOS_DEPLOYMENT_TARGET ${IOS_DEPLOYMENT_TARGET_TMP} CACHE STRING "xrOS Deployment Target")

set(IOS_EMBED_BITCODE OFF CACHE BOOL "Embed LLVM bitcode")
...
if (${SIMULATOR_ARCHS_NBR})
  configure_file(CMake/visionos.simulator.toolchain.cmake.in
    ${CMAKE_CURRENT_BINARY_DIR}/CMake/visionos.simulator.toolchain.cmake
    @ONLY
  )
  crosscompile(vtk-xros-simulator
    ${CMAKE_CURRENT_BINARY_DIR}/CMake/visionos.simulator.toolchain.cmake
  )
  set(VTK_GLOB_LIBS "${VTK_GLOB_LIBS} \"${INSTALL_DIR}/vtk-xros-simulator/lib/libvtk*.a\"" )
  list(APPEND IOS_ARCHITECTURES vtk-xros-simulator )
endif()

# for each device architecture
foreach (arch ${IOS_DEVICE_ARCHITECTURES})
  set(CMAKE_CC_ARCH ${arch})
  configure_file(CMake/visionos.device.toolchain.cmake.in
    ${CMAKE_CURRENT_BINARY_DIR}/CMake/visionos.device.toolchain.${arch}.cmake
    @ONLY
  )
  crosscompile(vtk-xros-device-${arch}
    ${CMAKE_CURRENT_BINARY_DIR}/CMake/visionos.device.toolchain.${arch}.cmake
  )
  set(VTK_GLOB_LIBS "${VTK_GLOB_LIBS} \"${INSTALL_DIR}/vtk-xros-device-${arch}/lib/libvtk*.a\"" )
  list(APPEND IOS_ARCHITECTURES vtk-xros-device-${arch} )
endforeach()
